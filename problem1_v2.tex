...
\usepackage{titlesec}
\usepackage{float}
\geometry{top=0.6in, right=0.65in, left=0.65in, bottom=0.6in}
...
$A\!=\!2$, $w\!=\!1$, $\alpha\!=\!\beta\!=\!2$, $\tau\!=\!0.4$ gives $h^*\!=\!0.30$, $h^{\mathrm{soc}}\!=\!0.40$, $h^{\mathrm{firm}}\!=\!0.50$. \textbf{Justification of the parameter choices.} (i) $\tau\!=\!0.4$ is the population-weighted midpoint between US ($0.35$) and EU ($0.56$) labour-tax wedges (OECD \emph{Taxing Wages 2023}, Q1 panel (c)). (ii) The hours scale anchors $h^*\!=\!0.30$ to BLS-OECD hours-per-worker over awake-time at the US point ($\sim\!2{,}000$ work hours / $\sim\!6{,}500$ awake hours $\approx\!0.31$, in line with Prescott 2004). (iii) $\alpha\!=\!\beta$ is a symmetry assumption that makes the welfare-improving band of \eqref{eq:capwelf} collapse to the textbook expression $(h^*,h^{\mathrm{firm}})$, but \textbf{only the ordering $h^*\!<\!h^{\mathrm{soc}}\!<\!h^{\mathrm{firm}}$ is load-bearing} for what follows; the qualitative results are invariant in $\alpha,\beta$ separately. (iv) $A\!=\!2,\,w\!=\!1$ are normalisations: the level of $A$ is identified only relative to $w$, and our results depend on the ratio $A/w$, not the absolute scale.
...
A cap $\bar h$ is a quantity instrument that takes the firm's labour-demand response as given. \emph{What the trade-off captures.} Workers value leisure (low $h^*$); firms value output (high $h^{\mathrm{firm}}$); a binding cap forces hours from $h^{\mathrm{firm}}$ toward $\bar h$. Whether the redistribution is welfare-improving depends on whether $\bar h$ lands inside or outside the welfare-improving band defined by joint-surplus concavity: too tight a cap overshoots and destroys joint surplus. \emph{What this buys.} A closed-form welfare condition that depends only on the gap $h^{\mathrm{firm}}\!-\!h^{\mathrm{soc}}$, transparent in the heterogeneity extension. \emph{What it costs.} (a) Extensive margin shut down (Q1 footnote): no firm response on number of bodies hired. (b) Wages held at the firm-power IR-binding level (no adjustment of $w$ to absorb the cap). (c) Heterogeneity on $\alpha$ only (workers differ in disutility curvature; we abstract from heterogeneous wages and outside options). The closest sufficient-statistics analogue is Lee--Saez (2012) on the minimum wage, but their welfare logic runs through extensive-margin rationing whereas a binding hours cap rations \emph{intensive}-margin only.
...
Full Lagrangian/FOC derivation of \eqref{eq:capwelf}, \eqref{eq:alpha-threshold}, and the aggregate condition is in Appendix~\ref{app:q4}.
...
\textbf{Reading.} A cap that pulls $h$ \emph{toward} the joint-surplus optimum is unambiguously welfare-improving (the green band of Figure~\ref{fig:simcap}c). A cap that pulls $h$ \emph{below} $h^{\mathrm{soc}}$ overshoots, and once $\bar h\!<\!2h^{\mathrm{soc}}-h^{\mathrm{firm}}\!=\!h^*\!=\!0.30$ even the worker is hurt --- joint surplus turns negative.
...
\begin{equation}
\alpha^*(\bar h) = \frac{2(1-\tau)w}{\bar h+h^{\mathrm{firm}}}.
...
\]
At baseline ($\bar\alpha\!=\!2$, $\alpha^*\!=\!1.333$ at $\bar h\!=\!h^{\mathrm{soc}}$) the cap is welfare-improving on average; a workforce with median $\alpha\!=\!1$ would lose. \emph{Bridge.} A cap helps if the average worker in a country has higher leisure preference than the threshold $\alpha^{*}$. If the workforce skews toward income-lovers ($\alpha_i$ low), the cap hurts more workers than it helps and the aggregate verdict flips.
...
\subsection{Modelling choice}
\emph{What the trade-off captures.} The Walrasian benchmark rewards productivity through wages, so a sectoral shock is absorbed by reallocating hours; the union changes the channel of absorption (wages take more, hours take less) and, under downward wage rigidity, makes that channel asymmetric across positive and negative shocks. \emph{What this buys.} A single bargain primitive ($\eta$) delivers a continuous comparative static between the firm-power baseline ($\eta\!=\!0$) and the worker-power limit ($\eta\!\to\!1$), nesting the Q4 distortion and the Walrasian benchmark as endpoints. \emph{What it costs.} (a) A single encompassing union, following Calmfors--Driffill (1988): industry-level unions internalise their macroeconomic effect, so their objective aligns with worker surplus rather than with subgroup rent extraction --- this rules out the sectoral inter-union competition story (Acemoglu--Pischke 1999), but lets us match the European centralised-bargaining institutional reality. (b) Right-to-manage instead of efficient bargaining: empirically, RTM dominates in Europe (MaCurdy--Pencavel 1986). (c) Informal labour and the public sector are outside the model.\footnote{An alternative model would have the union charge a share $s$ of worker income to finance itself; the union's revenue then scales with worker income, so its incentive maps directly onto worker surplus and the alignment result below is unchanged.}

\subsection{Two-sector economy: competitive benchmark}
Two sectors $j\!\in\!\{\textsc{g},\textsc{b}\}$ (good, bad) with productivities $A_{\textsc{g}}\!>\!A_{\textsc{b}}$, drawn from a mean-preserving sectoral shock around a symmetric pre-shock benchmark $A_{\rm pre}\!=\!2$ (we set $A_{\textsc{g}}\!=\!2.2$, $A_{\textsc{b}}\!=\!1.8$, a $\pm 10\%$ shock chosen to match the cross-sectoral TFP dispersion documented for OECD manufacturing in Bento--Restuccia (2017) and the manufacturing-vs-services productivity gap in Bils--Klenow (2000); Hsieh--Klenow (2009) report wider dispersion in developing economies, but $\pm 10\%$ is a defensible upper-middle estimate for the developed-world panels we have in mind). Within each sector a representative worker--firm pair has the quadratic primitives of \S 3.
...
\textbf{US vs.\ EU calibration.} OECD ICTWSS coverage maps US ($\sim\!12\%$) to $\eta^{\,US}\!\approx\!0$ and population-weighted EU coverage ($\sim\!60$--$70\%$ across DE/FR/IT/SE under \emph{erga omnes}) to $\eta^{\,EU}\!\approx\!0.5$. The implied union wage premium ($\approx\!24\%$) is in the upper range of Card--Cardoso--Heining--Kline (2018). At $\eta^{\,EU}\!=\!0.5$ we get $h_{\textsc{g}}\!\approx\!0.42$, $h_{\textsc{b}}\!\approx\!0.33$, an EU--US gap of order $\approx\!0.10$ on a US level of $0.50$ ($\approx\!20\%$ in logs) --- the right order of magnitude to absorb a substantial fraction of the gap documented in Q1.

\subsection{Sectoral shocks: union pass-through and asymmetry}
Substituting $A_{\textsc{g}}, A_{\textsc{b}}$ into \eqref{eq:rtm-bargain} at $\eta\!=\!0.5$ (Figure~\ref{fig:simshocks}): $w_{\textsc{g}}^{\,U}\!=\!1.354,\,h_{\textsc{g}}^{\,U}\!=\!0.423$ and $w_{\textsc{b}}^{\,U}\!=\!1.132,\,h_{\textsc{b}}^{\,U}\!=\!0.334$. The good sector splits the productivity gain into wages ($\Delta w/w\!\approx\!+25\%$) and reduced hours ($\Delta h/h\!\approx\!-24\%$); joint surplus is roughly preserved, only its split changes.
...
\subsection{Modelling choice}
The Q4 cap and the Q5 union, in our specification, are both quantity instruments that pull hours from $h^{\mathrm{firm}}$ toward $h^{\mathrm{soc}}$.\footnote{We focus on hours throughout. The wage channel is implicit: the union's effect on $h$ operates through a bargained wage premium, and the Saez--Stantcheva $\lambda$ assigns welfare value to the firm-to-worker transfer that this premium effects. We do not separately analyse cross-country wage differences (Krueger--Summers 1988; Card et al.\ 2018), which the prompt does not ask about and our partial-equilibrium calibration cannot speak to.} Whether a given configuration is welfare-enhancing depends on \emph{whose} welfare we are summing. The natural representation of the cross-country welfare disagreement is the Saez--Stantcheva (2016) generalised welfare weight: the planner attaches premium $\lambda\!\in\![0,1]$ to a worker dollar over a firm-owner dollar,
\[
...
We map the Nash-bargain weight $\eta$ to OECD bargaining-coverage data: $\hat\eta^{\,US}\!\approx\!0.15$ (coverage $\sim\!12\%$), $\hat\eta^{\,EU}\!\approx\!0.50$ (coverage $\sim\!60\%$ population-weighted across DE/FR/IT/SE).\footnote{The mapping is a coarse linear proxy: it overstates Nordic high-density regimes and understates French \emph{erga omnes} extension regimes, where coverage is high but bargaining power is weak. A wage-premium calibration via Card et al.\ (2018) gives a comparable band.} These two observations admit two competing readings.

\paragraph{Reading A --- common $\lambda$.} If both countries share the HSV-anchored $\lambda\!=\!0.30$, the unique welfare-optimum is $\eta^{*}(0.30)\!=\!0.58$. Both observed values are \emph{below} the optimum: the US is heavily under-unionised ($\hat\eta^{US}\!=\!0.15$), the EU mildly under ($\hat\eta^{EU}\!=\!0.50$). Welfare loss:
\vspace{-6pt}
\begin{center}\footnotesize
...
\midrule
US & 0.15 & 0.58 & under-unionised & 0.259 & 0.282 & $\mathbf{-0.022}$ \\
EU & 0.50 & 0.58 & mildly under & 0.281 & 0.282 & $\mathbf{-0.001}$ \\
\bottomrule
\end{tabular}
\end{center}
\vspace{-4pt}
Reading A delivers an unconditional verdict: \textbf{at HSV-implied fairness, EU institutions are essentially welfare-optimal; US institutions leave $\sim\!8\%$ of joint surplus on the table because the firm extracts too much.} Equivalently, the US is below the welfare-improving band of Q4 \eqref{eq:capwelf} \emph{from the union side}: a US planner that accepted $\lambda\!=\!0.30$ would push for higher union coverage.

\paragraph{Reading B --- country-specific $\lambda$.} Inverting $\eta^{*}(\lambda)$ on observed coverage:
\[
...
\]
Each country's institutions are then \emph{at-optimum given its own fairness primitive}; cross-country fairness-belief gap is $\Delta\lambda\!\approx\!0.19$. The hours drag from the union channel is small in the US ($h^{\mathrm{firm}}\!\to\!0.45$, $-9\%$) and larger in the EU ($h^{\mathrm{firm}}\!\to\!0.38$, $-24\%$). \textbf{This is the missing variable in Q1 panel (c)}: the cross-country hours variation that the noisy tax--hours relation cannot pin down is reconstructed by the $\lambda$-driven union channel.

\subsection{Verdict}
The two readings give compatible verdicts on \emph{whether} the EU system is more welfare-enhancing, but disagree on \emph{why}.
\begin{itemize}\setlength\itemsep{0pt}\setlength\parskip{0pt}\setlength\topsep{2pt}
\item \emph{Reading A} (common $\lambda\!=\!0.30$): \textbf{EU is unconditionally welfare-better}, because EU institutions are closer to any reasonable planner's optimum than US institutions are. The mechanism is institutional implementation, not preferences.
\item \emph{Reading B} (country-specific $\lambda$): \textbf{neither country is welfare-better in absolute terms} --- each implements its own optimum. The cross-country hours gap reflects a fairness-belief gap, which is the missing variable in the noisy Q1 cross-section.
\end{itemize}

The two readings are \emph{empirically distinguishable} (Figure~\ref{fig:simq7etalambda}): Reading A predicts all countries on the same horizontal line $\eta\!=\!\eta^{*}(0.30)$, which the observed range (US 0.15, FR 0.98, SE 0.88, DE 0.52) clearly violates. \textbf{The data prefer Reading B} --- but Reading A is the cleaner welfare argument.\footnote{The cap channel (Q4) is orthogonal at our calibration: the union alone pulls $h$ below any sensible cap, so Q4 is a counterfactual backstop, not an active driver. Outside the labour-market block, dominant US--EU welfare differences run through non-labour channels (life expectancy, consumption levels --- Jones--Klenow 2016); our verdict is sign-determinate but quantitatively second-order.}
...
Acemoglu, D., \& Pischke, J.-S.\ (1999). The Structure of Wages and Investment in General Training. \emph{QJE} 114(1) [\textbf{Q5 alternative: subgroup unions}].
...
Bento, P., \& Restuccia, D.\ (2017). Misallocation, Establishment Size, and Productivity. \emph{AEJ: Macro} 9(3) [\textbf{Q5 sectoral TFP dispersion}].
...
Bils, M., \& Klenow, P.\ J.\ (2000). Does Schooling Cause Growth? \emph{AER} 90(5) [\textbf{Q5 manufacturing vs services productivity gap}].
...
Card, D., Cardoso, A.\ R., Heining, J., \& Kline, P.\ (2018). Firms and Labor Market Inequality: Evidence and Some Theory. \emph{JOLE} 36(S1), S13--S70 [\textbf{Q7 wage-premium calibration}].
...
MaCurdy, T.\ E., \& Pencavel, J.\ H.\ (1986). Testing Between Competing Models of Wage and Employment Determination in Unionised Industries. \emph{JPE} 94(3) [\textbf{Q5 RTM vs efficient bargaining}].
...
\medskip
\noindent\emph{Replication scripts in \texttt{research/}: \texttt{build\_figure\_panel.py} (Q1 figure); \texttt{sim\_models\_v2.py} (Q5 RTM and sectoral shocks; Q7 \texttt{make\_q7\_eta\_lambda}); \texttt{sim\_mechanisms.py} (Q4 cap simulation).}
\endgroup
...
This appendix derives the cap surpluses, the welfare band \eqref{eq:capwelf}, the heterogeneous-worker threshold \eqref{eq:alpha-threshold}, and the aggregate condition from primitives via Lagrangian and FOC.
...
\textbf{Why IR enters the programme.} In the firm-power setup the firm chooses both the wage $w$ and the hours $h$. Without IR the problem would be degenerate: the firm could set $w$ arbitrarily low and extract unbounded rents, since the worker has no recourse. The IR constraint disciplines the wage offer --- the worker has an outside option $V_w^{0}$ (unemployment, home production, or another job) and refuses any contract that delivers less. In equilibrium the firm offers the lowest wage that still induces participation, so $V_w(h)=V_w^{0}$ exactly: IR binds and pins down $w$ as a function of $h$. Once $w$ is pinned, the firm's residual choice is $h$, which it sets at $h^{\mathrm{firm}}$ absent the cap. IR thus plays two roles: it defines the firm-power benchmark (the worker is held at her outside option, the firm captures all worker--firm surplus, so $V_w=V_w^{0}$ in equilibrium), and it provides the wage equation that closes the model when $w$ would otherwise be free. This is the standard monopsony / take-it-or-leave-it setup (Manning 2003); efficient bargaining (Q5 with $\eta>0$) replaces ``IR binds'' with a Nash-product split, but the structural role of IR --- determining how the worker--firm surplus is divided --- is the same.
...
\end{align}
using the factorisation $\bar h^{2}-(h^{\mathrm{firm}})^{2}=(\bar h-h^{\mathrm{firm}})(\bar h+h^{\mathrm{firm}})$. \eqref{eq:dVf}, \eqref{eq:dW}, \eqref{eq:dVw} are precisely the three rows of the table in \S 4.3.
...
\subsection{Heterogeneous-worker threshold: derivation of \texorpdfstring{\eqref{eq:alpha-threshold}}{Eq.\ (2)}}
Replace $\alpha$ by the type-specific $\alpha_i$ in \eqref{eq:dVw}:
\[
\Delta V_w^{i}(\bar h)=(\bar h-h^{\mathrm{firm}})\Bigl[(1-\tau)w-\tfrac{\alpha_i}{2}(\bar h+h^{\mathrm{firm}})\Bigr].
\]
Since $\bar h<h^{\mathrm{firm}}$, the leading factor is negative; therefore $\Delta V_w^{i}>0$ iff the bracket is also negative:
\[
(1-\tau)w<\tfrac{\alpha_i}{2}(\bar h+h^{\mathrm{firm}})
\;\Longleftrightarrow\;
\alpha_i>\frac{2(1-\tau)w}{\bar h+h^{\mathrm{firm}}}\equiv\alpha^{*}(\bar h),
\]
which is \eqref{eq:alpha-threshold}. At $\bar h=h^{\mathrm{soc}}=0.40$: $\alpha^{*}=2(0.6)(1)/(0.4+0.5)=4/3\approx1.333$.

\subsection{Aggregate welfare condition}
With type distribution $F$ and mean $\bar\alpha\equiv\int\alpha\,dF(\alpha)$, integrate \eqref{eq:dVw} (with $\alpha\mapsto\alpha_i$) and add \eqref{eq:dVf}:
\[
\overline{\Delta W}(\bar h)
\equiv\int\!\Delta V_w^{i}(\bar h)\,dF(\alpha)+\Delta V_f(\bar h)
=(\bar h-h^{\mathrm{firm}})\!\Bigl[(1-\tau)w-\tfrac{\bar\alpha}{2}(\bar h+h^{\mathrm{firm}})\Bigr]-\tfrac{\beta}{2}(h^{\mathrm{firm}}-\bar h)^{2}.
\]
Completing the square in $\bar h$ shows $\overline{\Delta W}(\bar h)>0$ iff $\bar h\in(2\hat h^{\mathrm{soc}}-h^{\mathrm{firm}},\,h^{\mathrm{firm}})$, where $\hat h^{\mathrm{soc}}\equiv(A-\tau w)/(\bar\alpha+\beta)$ is the social optimum at the population-mean disutility $\bar\alpha$. The band has the same structure as \eqref{eq:capwelf} with $\alpha\mapsto\bar\alpha$. At baseline ($\bar\alpha=2$, $\alpha^{*}=1.333$ at $\bar h=h^{\mathrm{soc}}$) the cap is welfare-improving on average; a workforce with median $\alpha=1$ would have $\hat h^{\mathrm{soc}}=(A-\tau w)/(\bar\alpha+\beta)=1.6/3\approx 0.53>h^{\mathrm{firm}}$, so any binding cap reduces aggregate welfare for that workforce.

% =========================================================================
\section{Formal derivation of the Q5 RTM bargain}\label{app:q5}
% =========================================================================

This appendix derives the right-to-manage Nash-bargain equilibrium of \S 4.3 from primitives via Lagrangian and FOC. It supplies (i) the closed-form bargain equation behind the curves $w_j(\eta), h_j(\eta)$, (ii) the boundary conditions at $\eta\!=\!0$ and $\eta\!\to\!1$, (iii) the comparative statics $\partial w/\partial\eta>0$, $\partial h/\partial\eta<0$, and (iv) the welfare-restorer claim. The derivation matches the equilibrium condition implemented in \texttt{research/sim\_models\_v2.py:rtm\_x} (annotated quadratic in lines 234--239 of that file).
...
Collecting terms:
\begin{equation}
\boxed{\;2B_v\,x_j^{2}-A_v(2-\eta)\,x_j+2(1-\eta)\,V_w^{0}=0.\;}
\label{eq:rtm-quad}
\end{equation}
The equilibrium $x_j^{\star}(\eta)$ is the larger root (the smaller root is the IR-binding fixed point at $\eta\!=\!0$ corner --- see below), and the bargained wage and hours follow as $w_j(\eta)=A_j-x_j^{\star}(\eta)$, $h_j(\eta)=x_j^{\star}(\eta)/\beta$. This is exactly the equation solved by \texttt{rtm\_x}; at calibration $(\alpha,\beta,\tau,A,V_w^{0})=(2,2,0.4,2,0.05)$ it reads $1.1x^{2}-0.6(2-\eta)x+0.1(1-\eta)=0$, with $x^{\star}(0)=1$ recovering $w\!=\!1$, $h\!=\!0.5$, and $x^{\star}(0.5)\!\approx\!0.846$ giving $w\!\approx\!1.154$, $h\!\approx\!0.423$ (Figure 3 numbers).
...
\]
\textbf{Note on the text's $\eta\!\to\!1$ statement.} \S 4.3 reads ``$h_j\to h_j^{*}=(1-\tau)w_j/\alpha$'' as $\eta\!\to\!1$. The exact RTM limit gives $h_j(1)/h^{*}(w_j(1))=\alpha/[\alpha+\beta(1-\tau)]<1$ (equal to $0.625$ at calibration). The reason: under RTM the firm \emph{always} chooses on its own labour demand $h^d(w)$, so the worker's preferred hours $h^{*}(w)=(1-\tau)w/\alpha$ are reached only at the wage $w^{**}\equiv\alpha A_j/[\alpha+\beta(1-\tau)]$ where the two schedules intersect. The union, optimising on the demand curve as $\eta\!\to\!1$, picks a wage \emph{above} $w^{**}$, so $h^d(w_{\eta=1})<h^{*}(w_{\eta=1})$. The ``welfare-restorer'' claim of \S 4.3 is unaffected --- $h_j(\eta)$ still falls monotonically through $h^{\mathrm{soc}}$ --- but the equality $h_j(1)=h^{*}_j$ should be replaced by the strict inequality $h_j(1)<h^{*}(w_j(1))$.
...
\]
The bracket equals $V_w'(x_j)\cdot$ (positive constant) at the larger root and is positive there (second-order condition for the Nash maximum). The right-hand side is unambiguously negative for $x_j>0,\,V_w^{0}\ge 0$. Hence
\[
...
\subsection{Sectoral shocks: derivation of the \S 4.4 numbers}
At calibration the bargain quadratic reads $1.1x^{2}-0.3A_j(2-\eta)x+0.1(1-\eta)=0$, with larger root
\(x^{\star}_j(\eta;A_j)=[0.3A_j(2-\eta)+\sqrt{0.09A_j^{2}(2-\eta)^{2}-0.44(1-\eta)}]/2.2\).

\textbf{(i) Three regimes per sector.} Evaluating at the two productivities $A_g=2.2$, $A_b=1.8$ and the pre-shock baseline $A_{\rm pre}=2$:
\vspace{-4pt}
\begin{center}\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lccccc}
\toprule
& $A_j$ & $\eta$ & $x_j^{\star}$ & $w_j$ & $h_j$ \\
\midrule
Free market (FM) -- good & 2.2 & 0 & 1.119 & 1.081 & 0.559 \\
Flexible union -- good   & 2.2 & 0.5 & 0.846 & 1.354 & 0.423 \\
Free market (FM) -- bad  & 1.8 & 0 & 0.878 & 0.922 & 0.439 \\
Flexible union -- bad    & 1.8 & 0.5 & 0.668 & 1.132 & 0.334 \\
Pre-shock union           & 2.0 & 0.5 & 0.758 & 1.242 & 0.379 \\
\bottomrule
\end{tabular}
\end{center}
\vspace{-4pt}
\noindent The good-sector union-vs-FM headlines follow as
\[
\frac{\Delta w_g}{w_g^{\rm FM}}=\frac{1.354-1.081}{1.081}=+25.2\%,\qquad
\frac{\Delta h_g}{h_g^{\rm FM}}=\frac{0.423-0.559}{0.559}=-24.4\%.
\]
Joint surplus at the good sector is $W=A_jh-\tau w h-\tfrac{\alpha+\beta}{2}h^{2}$; substituting yields $W^{\rm FM}_g=0.363$ and $W^{U}_g=0.344$, a $5\%$ shrinkage --- ``roughly preserved, only its split changes,'' as in \S 4.4.

\textbf{(ii) Rigid bad sector.} The KKT step adds a wage-floor multiplier $\mu_{\rm rig}\!\ge\!0$ on $w\ge w_{\rm pre}$, which binds in the bad sector ($w_b=w_{\rm pre}=1.242$). Hours are then determined off labour demand alone:
\[
h_b^{\rm rig}=(A_b-w_{\rm pre})/\beta=(1.8-1.242)/2=0.279.
\]
The good sector's floor is slack and the standard FOC applies. The headline asymmetries:
\[
\frac{h_b^{\rm rig}-h_b^{\rm FM}}{h_b^{\rm FM}}=\frac{0.279-0.439}{0.439}=-36.4\%,
\]
\[
V_f^{\rm FM,b}=\frac{(A_b-w_b^{\rm FM})^{2}}{2\beta}=\frac{0.878^{2}}{4}=0.193,\qquad
V_f^{\rm rig,b}=A_bh_b^{\rm rig}-\tfrac{\beta}{2}(h_b^{\rm rig})^{2}-w_{\rm pre}h_b^{\rm rig}=0.078,
\]
\[
\frac{V_f^{\rm rig,b}-V_f^{\rm FM,b}}{V_f^{\rm FM,b}}=\frac{0.078-0.193}{0.193}=-59.6\%,
\]
i.e.\ the $36\%$ hours drop and $60\%$ profit drop reported in Figure~\ref{fig:simshocks} and \S 4.4.

\textbf{Asymmetry.} In the good sector the constraint is slack and \eqref{eq:rtm-foc} applies: the productivity gain is split between a wage rent and an hours cut (the union ``absorbs'' the shock). In the bad sector $\mu_{\rm rig}>0$ and the wage cannot adjust: the entire shock flows through the labour-demand schedule into hours, hence the amplification.
