...
\noindent\emph{Detailed (unabridged) versions of \S A.1 and \S A.2 are reproduced in the companion document \texttt{appendix\_full.pdf} at \href{https://github.com/AlessandroCaggia30/overworked-americans-takehome/blob/main/appendix_full.pdf}{\texttt{github.com/AlessandroCaggia30/overworked-americans-takehome}}.}

\subsection{Primitives, FOCs, and the three bliss points}
Worker and firm payoffs $V_w(h)=(1-\tau)wh-\tfrac{\alpha}{2}h^{2}$ and $V_f(h)=Ah-\tfrac{\beta}{2}h^{2}-wh$ have FOCs
\[
h^{*}=(1-\tau)w/\alpha,\qquad h^{\mathrm{firm}}=(A-w)/\beta.
\]
The wage transfer cancels in $W\!\equiv\!V_w+V_f=Ah-\tau wh-\tfrac{\alpha+\beta}{2}h^{2}$, which maximises at
\[
h^{\mathrm{soc}}=(A-\tau w)/(\alpha+\beta).
\]
At calibration $(A,w,\tau,\alpha,\beta)=(2,1,0.4,2,2)$: $h^{*}=0.30,\,h^{\mathrm{soc}}=0.40,\,h^{\mathrm{firm}}=0.50$, the firm-power ordering.

\subsection{Firm's constrained programme under a cap}
The firm maximises $V_f(h)$ subject to $h\le\bar h$ and IR $V_w(h)\ge V_w^{0}$, with $w$ chosen to bind IR.\footnote{Without IR the firm-power problem is degenerate (firm pushes $w$ arbitrarily low for unbounded rents); IR pins $w$ at the level matching the outside option $V_w^{0}$.} The Lagrangian $\mathcal{L}=V_f(h)-\mu(h-\bar h)+\lambda(V_w(h)-V_w^{0})$, $\mu,\lambda\ge 0$, gives KKT conditions $V_f'(h)+\lambda V_w'(h)=\mu$, $\mu(h-\bar h)=0$, $\lambda(V_w-V_w^{0})=0$. With IR binding the wage is pinned, the hours problem reduces to $\max V_f$ s.t.\ $h\le\bar h$, and
\[
h^{\mathrm{eq}}(\bar h)=\min\{\bar h,\,h^{\mathrm{firm}}\},\qquad \mu=\max\{0,\,A-w-\beta\bar h\}.
\]
All subsequent surpluses are evaluated at this $h^{\mathrm{eq}}$ relative to the no-cap baseline $h^{\mathrm{firm}}$.
...
\]
At $\alpha=\beta$ the lower band-edge equals the worker's bliss $h^{*}$, so \eqref{eq:capwelf} simplifies to $\bar h\in(h^{*},h^{\mathrm{firm}})=(0.30,0.50)$.
