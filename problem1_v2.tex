...
\]
which is increasing on $[0,\hat\eta_j]$ (the union pulls hours \emph{toward} the social bliss) and decreasing on $[\hat\eta_j,1]$ (overshoot region). Worker surplus $V_w(h_j(\eta))$ is monotonically increasing throughout because the union is the worker's agent. A planner with any positive worker weight thus strictly prefers $\eta>0$ to $\eta=0$.

\subsection{Sectoral shocks: rigidity as a binding wage-floor}
For \S 4.4, substitute $A_g=2.2$ and $A_b=1.8$ into \eqref{eq:rtm-quad} at $\eta=0.5$. \emph{Flexible} bargain: numerical roots give $w_g^{U}\!\approx\!1.354$, $h_g^{U}\!\approx\!0.423$ and $w_b^{U}\!\approx\!1.132$, $h_b^{U}\!\approx\!0.334$. \emph{Rigid} bargain: replace the multiplier $\mu_f$ on firm IR by a wage-floor multiplier $\mu_{\mathrm{rig}}$ on $w\ge w_{\mathrm{pre}}\equiv w_j(\eta;A_{\rm pre})$. KKT: in the bad sector the constraint binds ($\mu_{\mathrm{rig}}>0$, $w_b=w_{\mathrm{pre}}$), the FOC \eqref{eq:rtm-foc} no longer holds with equality, and hours are determined off the firm's labour-demand schedule alone, $h_b^{\mathrm{rig}}=(A_b-w_{\mathrm{pre}})/\beta$. In the good sector the constraint is slack and the standard FOC \eqref{eq:rtm-foc} applies. Asymmetry follows: positive shocks pass through diluted (union absorbs them in wage rents via the unconstrained FOC), negative shocks pass through amplified (rigidity prevents the wage from absorbing the shock; hours collapse on the demand curve at the stuck wage).
