...
\]
\emph{SOC at the limit.} Evaluating the bracket $4B_v x_j-A_v(2-\eta)$ at $(x,\eta)=(A_v/(2B_v),1)$ gives $4B_v\!\cdot\!A_v/(2B_v)-A_v\!=\!2A_v-A_v\!=\!A_v>0$, so the second-order condition for the Nash maximum holds at the limit and $x_j^{\star}(1)$ is indeed the maximiser, not a corner. The ratio $h_j(1)/h^{*}(w_j(1))=\alpha/[\alpha+\beta(1-\tau)]<1$ ($0.625$ at calibration), confirming the strict inequality stated in \S 4.3: under RTM the firm responds on its own labour demand $h^d(w)$, so the worker's bliss $h^{*}(w)=(1-\tau)w/\alpha$ is reached only at the schedule-intersection wage $w^{**}\!\equiv\!\alpha A_j/[\alpha+\beta(1-\tau)]$, while the union as $\eta\!\to\!1$ picks a wage strictly above $w^{**}$.
