% Taxation in United Kingdom for the year 2015

function[inctax_p,inctax_s,socsec_p,socsec_s,benefit,netincome,emp_socsec]=tax_uk_2015(wage_principal,wage_spouse,married,children)

%************************** EXECUTE THE PARAMETER FUNCTION ************************

[basic_all,socseclimit1,socseclimit2,socsec_r1,socsec_r2,socsec_r3,taxbracket1,taxbracket2, tax_r1,tax_r2,tax_r3,child_ben1,child_ben2,singlebenefit,ssc_r2,st,wtc_basic,wtc_couplelone,wtc_30hr,ctc_family,ctc_child,ntc_1st_thres,ntc_1st_taper,basic_thres,cb_1st_thres,cb_2nd_thres,cb_taper1,cb_taper2, CB_first, CB_others]=uk_parameters_2015;
...
if children>0
    benefit=(366/7)*(child_ben1 + child_ben2* max(0,(children-1)))*(3/4);
        if wage_principal<cb_1st_thres
            benefit=benefit+(366/7)*(child_ben1 + child_ben2* max(0,(children-1)))*(1/4);
        elseif wage_principal>=cb_1st_thres && wage_principal<cb_2nd_thres
            benefit=benefit+((1-(wage_principal-cb_1st_thres)/(cb_taper2/cb_taper1)))*(366/7)*(CB_first+CB_others*max(0,(children-1)));
        else 
            benefit=benefit+0;
        end
end
