...
function[socsec, ssc_empr]=fr_socsec_2013(wage_principal,wage_spouse,pension_r,penex_r,penex_r2,penwidow_r,ssc_ceil,sick_r,unemp_r1,pens_empr1,pens_empr2,sick_empr,unemp_empr,accid_empr,smic,fam_empr,penex_empr,penex2_empr,others_empr1,others_empr2,ssc_empr_redr2,ssc_empr_red_max,ssc_empr_smic_ref,minrevtp,csa,cice_red,cice_max)
...
pension_empr=(pens_empr1+others_empr1)*min(wage_principal,ssc_ceil)+(pens_empr1+others_empr1)*min(wage_spouse,ssc_ceil)+(others_empr2)*max(0,wage_principal-ssc_ceil)+(others_empr2)*max(0,wage_spouse-ssc_ceil)+penex_empr*min(wage_principal,ssc_ceil)+ penex2_empr* max(0,min(wage_principal-ssc_ceil,2*ssc_ceil)) +penex_empr*min(wage_spouse,ssc_ceil)+ penex2_empr* max(0,min(wage_spouse-ssc_ceil,2*ssc_ceil));

%Illness , family, accidents,
illness_empr=(sick_empr+fam_empr+accid_empr+csa+pens_empr2)*wage_principal+(sick_empr+fam_empr+accid_empr+csa+pens_empr2)*wage_spouse;
