...
function[socsec, ssc_empr]=fr_socsec_2005(wage_principal,wage_spouse,pension_r,penex_r,penex_r2,penwidow_r,ssc_ceil,sick_r,unemp_r1,unemp_r2,pens_empr1,pens_empr2,sick_empr,unemp_empr,accid_empr,smic,fam_empr,penex_empr,penex2_empr,others_empr1,others_empr2,ssc_empr_redr1,ssc_empr_redr2,ssc_empr_red_max,ssc_empr_smic_ref,minrevtp)
...
if wage_principal<=ssc_empr_smic_ref*smic && wage_principal>0
    ssc_red_empr_principal=round(min(ssc_empr_red_max*wage_principal,max(0,min(smic+1-wage_principal,1))*(wage_principal*ssc_empr_redr2)+max(0,min(wage_principal-smic,1))*(ssc_empr_smic_ref*smic-wage_principal)*ssc_empr_redr1*min(wage_principal/minrevtp,1)));
else
    ssc_red_empr_principal=0;
end

if wage_spouse<=ssc_empr_smic_ref*smic && wage_spouse>0
    ssc_red_empr_spouse=round(min(ssc_empr_red_max*wage_spouse,max(0,min(smic+1-wage_spouse,1))*(wage_spouse*ssc_empr_redr2)+max(0,min(wage_spouse-smic,1))*(ssc_empr_smic_ref*smic-wage_spouse)*ssc_empr_redr1*min(wage_spouse/minrevtp,1)));
else
