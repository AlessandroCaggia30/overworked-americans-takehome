...
% Employees social security contributions 
 ...
 function [finalsocsec,empr_socsec] = ne_socsec_2002(wage_principal,wage_spouse,tax_cr_total,married,taxable_income,taxable_income_s,unemp_r,ssc_ceil,unemp_fran,med_limit,health_ceil,health_r,health_fixed,oldpen_r,widpen_r,ex_med_r,taxbracket2,rate_ssc,unemp_empr_r1,unemp_empr_r2,unemp_empr_fran1,inv_empr_r,employer_health,employer_health_s)
    ...
    unemployment_s = max(0,( min(wage_spouse,ssc_ceil)-unemp_fran ) )*unemp_r;     
     ...
     if wage_principal<=med_limit && wage_principal>0
            health_ins=min(wage_principal,health_ceil)*health_r + health_fixed;
     else
         health_ins=0;
     end
        
      if wage_spouse<=med_limit
            health_ins_s=min(wage_spouse,health_ceil)*health_r +health_fixed;
     else
         health_ins_s=0;
      end
