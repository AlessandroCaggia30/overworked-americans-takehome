...
% work_expense_all_s=min(max((work_rate*wage_spouse),work_min),work_max);
% 
% ss_unemp_s=max(0,unemp_r*(min((wage_spouse*(1-comp_rate)),ssc_ceil)-unemp_fran));  % Deduction for social security contr.
% 
% 
% 
% if wage_spouse<=med_limit
%     ss_med_empr_s=max(0,med_empr*min((wage_spouse*(1-comp_rate)),health_ceil)); % Employers health contribution increases taxable income
% else
%     ss_med_empr_s=0;
% end
% 
% 
% if wage_spouse>0
%     taxable_inc_s=wage_spouse-basic_all +(ss_med_empr_s-work_expense_all_s-ss_unemp_s);
% else
%     taxable_inc_s=0;
% end
