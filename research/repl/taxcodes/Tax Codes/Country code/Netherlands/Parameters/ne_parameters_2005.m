

% Parameter Assignment 

function[unemp_r,ssc_ceil,unemp_fran,med_limit,med_empr,health_ceil,health_r,health_fixed,oldpen_r,widpen_r,ex_med_r,taxbracket1,taxbracket2,taxbracket3,tax_r1,tax_r2,tax_r3,tax_r4,comb_credit_fr,comb_credit,singlepar_cr,addsinglepcr_r,gen_cred,emp_cred,work_cred1,work_cred2,emp_cr_thr,work_max,ch_cr_thr,adch_thr,adch_thr2,ch_cr,adch_cr,adch_cr2,rate_cr,rate_ssc,ch_ben,adcomb_cred,unemp_empr_r1,unemp_empr_r2,unemp_empr_fran1,inv_empr_r] = ne_parameters_2005

%Social Security Parameters
unemp_r=0.0585;    %unemployment contribution rate for gross earnings between 
ssc_ceil=43578;   %Social Security Ceiling
unemp_fran=15138;   %Unemployment contribution lower bound for wage

med_limit=33000;
med_empr=0.0675;
health_r=0.0145; %Med insurance rate 
health_ceil=29751;   %Medical insurance net wage ceiling      
health_fixed= 388;   %basic insurance

 oldpen_r=0.179;   %Old age pension rate for 1st &2nd tax bracket
 widpen_r=0.0125;   %Widows and Orphans pension rate for 1st &2nd tax bracket
 ex_med_r=0.1345;  %Exceptional medical expenses and disability rate for 1st &2nd tax bracket

%Employers Social Security Parameters 
unemp_empr_r1=0.0245;
unemp_empr_r2=0.0175;
unemp_empr_fran1=15138;
inv_empr_r=0.0725;
 
 %Tax Schedule Parameters
taxbracket1=16893;
taxbracket2=30357;
taxbracket3=51762;
tax_r1=0.018;
tax_r2=0.0935;   
tax_r3=0.42;
tax_r4=0.52;
  
%Tax credit parameters
 gen_cred=1894;
 
 work_cred1=0.01753;
  emp_cred=144;
 work_cred2=0.11213;
  emp_cr_thr=8101;
  work_max=1287;

 ch_cr=112;
 ch_cr_thr=60447;  
 
 adch_thr=28491;
 adch_cr=690; 
 
 adch_thr2=30225;
 adch_cr2=504;  
 
comb_credit=228; 
 comb_credit_fr=4366; 
adcomb_cred= 389;


singlepar_cr=1401;
addsinglepcr_r=0.043;


rate_cr= 1.8/(1.8+32.6);
rate_ssc= 1-rate_cr;


ch_ben=1716/2;



end