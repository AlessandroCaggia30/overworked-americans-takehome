...
supp_oneperson=3584; %supplement for one person households
supp_soleearner=7168; %1 earner couples
single_par_all=5735; % single parent allowance (sing_par_sup)
single_par_all_two=4333; %extra single parent supplement for parents with kids younger than 12
work_supplement=397;
extrawork_supplement=768;
...
unemp_r=0.028;    %unemployment contribution rate for gross earnings between 15921-46205
inv_r=0.16;     %invalidity contribution rate.(zero from 1998 onwards)
inv_fran=23751;  % Invalidity franchise


med_limit=48100;     %Health insurance wage limit
health_ceil=40977;   %Medical insurance net wage ceiling(med_ceil)
 health_r=0.046;      %Med insurance rate(med_rate)
 
 med_empr=0.046;     %Employer health contribution rate
 oldpen_r=0.117;   %Old age pension rate for 1st &2nd tax bracket(old_rate)
 widpen_r=0.014;   %Widows and Orphans pension rate for 1st &2nd tax bracket(wid_rate)
 gen_ceil=63200; %Ceiling for General Schemes

  
 %Employers Social Security Parameters 
unemp_empr_r1=0.0075;
sick_empr_r=0.05;
inval_r_empr=0.0015;
...
tax_r9=0.72;
 ...
 child_ben=3068;   % child benefit for 2 children (ch2_trans)
