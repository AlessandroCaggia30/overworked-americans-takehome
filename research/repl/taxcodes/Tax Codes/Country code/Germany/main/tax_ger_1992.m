% Taxation in the German system for the year 1992  polyzig@yahoo.com

function [inctax,socsec_couple, netincome,ssc_empr] = tax_ger_1992(wage_principal, wage_spouse, married, children, taxyear)



%************************** execute the Parameter functions *************************************************************************************************

	[insurable_ceiling, insurable_ceiling2, pension_rate, sickness_rate, unemployment_rate, tax_free_inc, soc_sec_lim ]=ger_parameters_socsec_1992;
	...
	[ wr_all,ss_rate, child_all, child_credit, child_credit2,child_credit3, child_credit4, net_inc_ceiling, net_inc_ceiling2, ...
        net_inc_ceiling_child, add_child_transfer, child_credit_spec, ssc_limit_sing, ssc_limit_coup] = ger_parameters_taxableinc_1992;
...
% *************************************************************************
  ...
  if (children > 0) && (inctax < 1)
	...
	if married == 0
		inctax = inctax - children_credit ;
	else
		inctax =  2*(inctax - (children_credit/2)) ;
