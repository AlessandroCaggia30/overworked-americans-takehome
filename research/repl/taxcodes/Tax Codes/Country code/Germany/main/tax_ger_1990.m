%  Taxation in the German system for the year 1991  polyzig@yahoo.com

function [inctax,socsec_couple, netincome,ssc_empr] = tax_ger_1990(wage_principal, wage_spouse, married, children, taxyear)



%************************** execute the Parameter functions *************************************************************************************************

	[insurable_ceiling, insurable_ceiling2, pension_rate, sickness_rate, unemployment_rate, tax_free_inc, soc_sec_lim ]=ger_parameters_socsec_1990 ;
	[tx1, tx3, tx4, tx6, tx7, tx8, tx9, tx10, tx11, tx12, tx13]=ger_parameters_tax_1990_1993;
	[ wr_all,ss_rate, child_all, child_credit, child_credit2,child_credit3, child_credit4, net_inc_ceiling, net_inc_ceiling2, ...
        net_inc_ceiling_child, add_child_transfer, child_credit_spec, ssc_limit_sing, ssc_limit_coup] = ger_parameters_taxableinc_1990;
...
%********************************************** 2)  TAX DEDUCTIONS   *********************************************************************************************
	...
	[taxable_inc_child, ss_ded_p] = ger_taxableinc_1990_1993(wage_principal, wage_spouse, socsec_p,socsec_couple, married, children, taxyear, ...
        Avg_wage_couple, ss_rate, wr_all, child_all, ssc_limit_sing, ssc_limit_coup);
	...
	if net_inc > (net_inc_ceiling + net_inc_ceiling_child*children)
		if children < 2
 		...
 		elseif children < 4
                ...
                children_credit = max((2*(children - 2)*child_credit_spec + child_credit + child_credit_spec),((child_credit3 + child_credit2 + ...
                    child_credit) + (children-3)*child_credit4 - 0.5*((net_inc-(net_inc_ceiling + net_inc_ceiling_child*children)))));
		end
	else 
		if children < 2
      			  children_credit = children*child_credit;
