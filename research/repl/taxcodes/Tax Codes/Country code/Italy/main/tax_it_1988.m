% Taxation in Italy in year 1988

function[inctax,local_tax,socsec,benefit,netincome,netinc_wobenefit]=tax_it_1988(wage_principal,wage_spouse,married,children)
...
%note: 0.86 is the value of (tax_inc_pr1/awp) in 1996. Here, I use 0.90 as an approximation to match the tables. There is no info available for years prior to 1996. 
if married==1 && children>0
    transfer1=((wage*0.90)>trans_sch_m_thresh1 && (wage*0.90)<=trans_sch_m_thresh2)*trans_sch_m1_c2+((wage*0.90)>trans_sch_m_thresh2 && (wage*0.90)<=trans_sch_m_thresh3)*trans_sch_m2_c2+((wage*0.90)>trans_sch_m_thresh3 && (wage*0.90)<=trans_sch_m_thresh4)*trans_sch_m3_c2+((wage*0.90)>trans_sch_m_thresh4 && (wage*0.90)<=trans_sch_m_thresh5)*trans_sch_m4_c2+((wage*0.90)>trans_sch_m_thresh5 && (wage*0.90)<=trans_sch_m_thresh6)*trans_sch_m5_c2+((wage*0.90)>trans_sch_m_thresh6 && (wage*0.90)<=trans_sch_m_thresh7)*trans_sch_m6_c2+((wage*0.90)>trans_sch_m_thresh7 && (wage*0.90)<=trans_sch_m_thresh8)*trans_sch_m7_c2+((wage*0.90)>trans_sch_m_thresh8 && (wage*0.90)<=trans_sch_m_thresh9)*trans_sch_m8_c2+((wage*0.90)>trans_sch_m_thresh9)*trans_sch_m9_c2 ;
else
    transfer1=0;
end
