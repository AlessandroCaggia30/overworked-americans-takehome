...
if children==0
    ...
    if married==1
        ...
        child_cr_p=max(child_cr*(95000-taxable_inc_p)/95000,((taxable_inc_p<15000)*(800-110*taxable_inc_p/15000)+(taxable_inc_p<=80000 &&taxable_inc_p>40000)*(690* (80000-taxable_inc_p)/40000)+(taxable_inc_p<=40000 &&taxable_inc_p>=15000)*((taxable_inc_p<spouse_cr_bracket2)*spouse_cr1+(taxable_inc_p>=spouse_cr_bracket2&& taxable_inc_p<spouse_cr_bracket3)*spouse_cr2+(taxable_inc_p>=spouse_cr_bracket3&& taxable_inc_p<spouse_cr_bracket4)*spouse_cr3+(taxable_inc_p>=spouse_cr_bracket4&& taxable_inc_p<spouse_cr_bracket5)*spouse_cr4+(taxable_inc_p>=spouse_cr_bracket5&& taxable_inc_p<spouse_cr_bracket6)*spouse_cr5+(taxable_inc_p>=spouse_cr_bracket6&& taxable_inc_p<spouse_cr_bracket7)*spouse_cr6+(taxable_inc_p>=spouse_cr_bracket7&& taxable_inc_p<spouse_cr_bracket8)*spouse_cr7+(taxable_inc_p>=spouse_cr_bracket8&& taxable_inc_p<spouse_cr_bracket9)*spouse_cr8+(taxable_inc_p>=spouse_cr_bracket9&& taxable_inc_p<spouse_cr_bracket10)*spouse_cr9+(taxable_inc_p>=spouse_cr_bracket10)*spouse_cr10)))+((children-1)*child_cr*(110000-taxable_inc_p)/110000);
    end
end

%Total tax credit
tax_cr_p= min(emp_cr_p+spouse_cr+child_cr_p, inctax_p_gr);
tax_cr_s= min(emp_cr_s+child_cr_s, inctax_s_gr);

%Income taxes net of tax credits
inctax_p=max(0,inctax_p_gr-tax_cr_p)+sol_tax_p-fiscal_b_p;
inctax_s=max(0,inctax_s_gr-tax_cr_s)+sol_tax_s-fiscal_b_s;

inctax=round(inctax_p+inctax_s);

%Local Tax
if taxable_inc_p<12000
    local_tax_p=0;
else
    local_tax_p=(taxable_inc_p)*local_rt;
end

if taxable_inc_p>=35000
    local_tax_p=local_tax_p+15000*reg_rt+(taxable_inc_p-15000)*reg_rt_2;
elseif taxable_inc_p<35000
    local_tax_p=local_tax_p+(taxable_inc_p)*reg_rt;
end

if taxable_inc_s<12000
    local_tax_s=0;
else
    local_tax_s=(taxable_inc_s)*local_rt;
end

if taxable_inc_s>=35000
    local_tax_s=local_tax_s+15000*reg_rt+(taxable_inc_s-15000)*reg_rt_2;
elseif taxable_inc_s<35000
    local_tax_s=local_tax_s+(taxable_inc_s)*reg_rt;
end
