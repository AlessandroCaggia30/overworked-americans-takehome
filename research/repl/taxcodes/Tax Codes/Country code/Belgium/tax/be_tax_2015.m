% Income Tax Calculation in Belgium for year 2015

function[finaltaxpaid,local_tax,finaltax_beforecr,family_credit,spouse_credit,finaltaxpaid_init,region_tax_init,region_tax]=be_tax_2015(wage_principal ,wage_spouse,married,children,ssc_p ,ssc_s ,red_p,red_s,workall_p , workall_s,taxbracket1,taxbracket2,taxbracket3,taxbracket4,tax_r1,tax_r2,tax_r3,tax_r4,tax_r5,quote_r,quote_max,child_cr1,child_cr2,child_cr3,child_cr4,child_cr5,s_parent_cr,single_cr,married_cr,supp_cr,supp_cr_thrsh,local_r,basicr_b,basicr_1,basicr_2,basicr_3,basicr_4, lic_r, lic_max,add_local_r,reg_r, red_r)
...
family_credit=round( min(family_c,taxbracket1)*tax_r1   +min(max(0,family_c-taxbracket1),(taxbracket2-taxbracket1)) *tax_r2      + min(max(0,family_c-taxbracket2),(taxbracket3-taxbracket2))*tax_r3         + min(max(0,family_c-taxbracket3),(taxbracket4-taxbracket3))*tax_r4       +max(0,family_c-taxbracket4)*tax_r5 );
spouse_credit=round( min(spouse_c,taxbracket1)*tax_r1   +min(max(0,spouse_c-taxbracket1),(taxbracket2-taxbracket1)) *tax_r2  + min(max(0,spouse_c-taxbracket2),(taxbracket3-taxbracket2))*tax_r3     + min(max(0,spouse_c-taxbracket3),(taxbracket4-taxbracket3))*tax_r4       +max(0,spouse_c-taxbracket4)*tax_r5);
...
%Tax  After Credit
finaltaxpaid_init=(1-red_r)*max(0,(finaltax_beforecr - family_credit - spouse_credit));
finaltaxpaid=round(finaltaxpaid_init-basic_cred_p -basic_cred_s);

%Regional Tax
%red_r=0.2599;

%Regional Tax
region_tax_init=finaltaxpaid_init;
region_tax=round(region_tax_init*reg_r);

%Local Tax
local_tax_init=round((local_r+add_local_r)*(finaltaxpaid_init+region_tax));
